...
def P(s):
    """Месячный pd.Period из строки вида '2023-01'."""
    return pd.Period(s, freq="M")


def make(rows):
    """Собирает таблицу продаж из списка кортежей (sku, location, period, qty)."""
...
def series(values, start="2023-01", sku="A", loc="MSK"):
    """Один непрерывный месячный ряд с заданными значениями."""
    periods = pd.period_range(start, periods=len(values), freq="M")
    return make([(sku, loc, str(p), v) for p, v in zip(periods, values)])


def flagged(df):
    """Добавляет пустую колонку flag, которую ожидают шаги после drop_duplicates."""
    df = df.copy()
    df["flag"] = ""
    return df
...
def test_exact_duplicate_dropped_not_summed():
    """Полный дубль удаляется, продажи не удваиваются."""
...
def test_conflicting_duplicate_is_masked_not_guessed():
    """Дубль ключа с разным qty маскируется в NaN, а не суммируется."""
...
def test_negative_becomes_nan_not_zero():
    """Отрицательная продажа становится NaN, а не ложным нулём."""
...
def test_missing_month_is_nan_not_zero():
    """Отсутствующий месяц внутри ряда — NaN с flag='missing_row'."""
...
def test_leading_months_before_launch_not_created():
    """До первой продажи строки не создаются: у новинки нет фальшивой истории."""
...
def test_discontinued_series_marked_inactive():
    """Ряд без продаж дольше INACTIVE_MONTHS помечается неактивным."""
...
def test_unit_shift_detected_and_rescaled():
    """Скачок уровня в 1000 раз распознаётся, старая история пересчитывается."""
...
def test_promo_spike_is_not_unit_shift():
    """Одиночный промо-пик не считается сменой единицы."""
...
def test_intermittent_series_is_not_unit_shift():
    """Редкий спрос с нулями не считается сменой единицы."""
    y = [0, 0, 5, 0, 0, 0, 40, 0, 0, 6, 0, 0]
    assert cl.detect_unit_shift(np.array(y, float)) is None


def test_stockout_zero_is_masked_but_true_zero_stays():
    """Ноль при out-of-stock маскируется, ноль без дефицита остаётся."""
...
def test_promo_masked_on_all_locations():
    """Промо по sku маскируется на всех локациях."""
...
def test_impute_never_negative_and_no_nan_left():
    """После импутации нет NaN и отрицательных значений."""
...
def test_clean_pipeline_end_to_end():
    """Все шаги вместе: ряд с пятью дефектами очищается до исходного уровня."""
